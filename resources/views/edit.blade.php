    ...
    <form class="xl:px-20" action="{{ route('permintaan.approval', $data->id)}}" method="post">
      ...
      <div id="alert-border-4" class="hidden items-center p-4 mb-4 text-yellow-800 border-t-4 border-yellow-300 bg-yellow-50 dark:text-yellow-300 dark:bg-gray-800 dark:border-yellow-800" role="alert">
        ...
        <div class="ms-3 text-sm font-medium">
          Please fill all the field before adding the row.
        </div>
      ...
      <div class="grid gap-6 mb-10 md:grid-cols-2 lg:grid-cols-9">
        ...
        <div class="flex items-end md:col-span-1 col-start-2 md:justify-start justify-end">
          <button id="add" type="button" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-20 sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Add</button>
        </div>
      </div>
      @endif
    ...
    <div id="crud-modal" tabindex="-1" data-modal-backdrop="static" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
      <div class="relative p-4 w-full max-w-md max-h-full">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
          ...
          <div id="alert-border-5" class="hidden items-center p-4 mb-4 text-yellow-800 border-t-4 border-yellow-300 bg-yellow-50 dark:text-yellow-300 dark:bg-gray-800 dark:border-yellow-800" role="alert">
            <svg class="flex-shrink-0 w-4 h-4" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
              <path d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5ZM9.5 4a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3ZM12 15H8a1 1 0 0 1 0-2h1v-3H8a1 1 0 0 1 0-2h2a1 1 0 0 1 1 1v4h1a1 1 0 0 1 0 2Z" />
            </svg>
            <div class="ms-3 text-sm font-medium">
              Please fill all the field before Edit the row.
            </div>
            <button type="button" class="ms-auto -mx-1.5 -my-1.5 bg-yellow-50 text-yellow-500 rounded-lg focus:ring-2 focus:ring-yellow-400 p-1.5 hover:bg-yellow-200 inline-flex items-center justify-center h-8 w-8 dark:bg-gray-800 dark:text-yellow-300 dark:hover:bg-gray-700" data-dismiss-target="#alert-border-5" aria-label="Close">
              <span class="sr-only">Dismiss</span>
              <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" />
              </svg>
          ...
          <div class="p-4 md:p-5">
            ...
            <div class="flex justify-end">
              <button id="save-edit" type="button" class="text-white inline-flex items-center bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                Edit
              </button>
            </div>
          </div>
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    const alertDiv = document.getElementById('alert-border-4');
    const alertText = document.querySelector('#alert-border-4 .text-sm');
    const alertDiv2 = document.getElementById('alert-border-5');
    let dataArray = [];
    const dismissButton = document.querySelector('[data-dismiss-target="#alert-border-4"]');
    const dismissButton2 = document.querySelector('[data-dismiss-target="#alert-border-5"]');
    let count = 0;
    ...
    dismissButton.addEventListener('click', function() {
      // Toggle 'hidden' class to show/hide the alert div
      alertDiv.classList.remove('flex');
      alertDiv.classList.add('hidden');
    });

    dismissButton2.addEventListener('click', function() {
      // Toggle 'hidden' class to show/hide the alert div
      alertDiv2.classList.remove('flex');
    ...
    function saveEdit(e) {
      const spec = document.getElementById('spec2').value.trim();
      const jumlah = document.getElementById('jumlah2').value.trim();
      const satuan = document.getElementById('satuan2').value.trim();
      const tanggal = document.getElementById('datepicker-autohide2').value.trim();
      const keterangan = document.getElementById('keterangan2').value.trim();

      if (!spec || !jumlah || !satuan || !tanggal || !keterangan) {
        // Display alert if any field is not filled
        alertDiv2.classList.remove('hidden');
        alertDiv2.classList.add('flex');
        alertDiv2.classList.add('opacity-100');
        alertDiv2.scrollIntoView({
          behavior: 'smooth',
          block: 'center'
        });
    ...
    function addRow() {
      ...
      const tanggal = document.getElementById('datepicker-autohide').value.trim();
      ...
      if (!spec || !jumlah || !satuan || !tanggal || !keterangan) {
        ...
        alertDiv.classList.add('flex');
        alertDiv.classList.add('opacity-100');
        alertText.textContent = 'Please fill all the field before adding the row'
        alertDiv.scrollIntoView({
          behavior: 'smooth',
          block: 'center'
        });
      ...
      if (count > 4) {
        ...
        alertDiv.classList.add('flex');
        alertDiv.classList.add('opacity-100');
        alertText.textContent = 'Maximal data of 5.';
        alertDiv.scrollIntoView({
          behavior: 'smooth',
          block: 'center'
        });
